...
public class Item
{
    const int TotalItems = 112;

    static readonly int _offsetItemTypes = OffsetScanner.GetOffset(GameOffset.FFX_ItemTypes);
    static readonly int _offsetItemCounts = OffsetScanner.GetOffset(GameOffset.FFX_ItemCounts);
    ...
    public static Item FromID(int itemID) => Items.FirstOrDefault(item => item.ID == itemID);
